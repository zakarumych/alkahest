...
fn get_owned_bounds(attrs: &[AlkahestAttr], generics: &syn::Generics) -> syn::WhereClause {
    for attr in attrs {
        match attr {
            AlkahestAttr::Bounds(bounds) => return bounds.clone(),
            AlkahestAttr::Owned(bounds) => {
                if let Some(bounds) = bounds {
                    return bounds.clone();
                }
            }
    ...
    syn::WhereClause {
        ...
        predicates: generics
            ...
            .filter_map(|param| -> Option<syn::WherePredicate> {
                if let syn::GenericParam::Type(param) = param {
                    let ident = &param.ident;
                    Some(syn::parse_quote!(#ident : ::alkahest::OwnedSchema))
                } else {
                    None
                }
            })
...
fn should_impl_owned<'a>(attrs: &[AlkahestAttr]) -> bool {
    for attr in attrs {
        if let AlkahestAttr::Owned(_) = attr {
            return true;
        }
    }
    false
